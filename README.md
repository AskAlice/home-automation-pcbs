...
- [`3dmodels/`](3dmodels) — STEP + WRL models (KiCad packages3D, Espressif
  kicad-libraries, EasyEDA/LCSC) referenced from every PCB
  (`${KIPRJMOD}/../../3dmodels/…`) + `manifest.json` (the resolved model and
  transform per footprint; re-fetch + re-attach: `python3 tools/attach_3d.py`).

> **Note:** datasheet PDFs and STEP/WRL models are not committed to git — they
> are regenerable with `tools/fetch_datasheets.py` and `tools/attach_3d.py`.
> The render PNGs/GIFs in each board folder *are* committed so the READMEs
> display without any tooling.
...
- `tools/render_pcb.py` — dependency-free PCB rasterizer (the render PNGs in
  each board folder).
- `tools/attach_3d.py` — fetches every footprint's 3D model (declarative
  footprint → source table) and rebuilds the `(model …)` blocks in all boards,
  aligning each body to the repo's custom footprint by pad bounding box.
  Needs `easyeda2kicad` (pip) for the LCSC parts.
- `tools/render3d.sh` — raytraced 3D renders (`render3d_top/bottom.png` per
  board) via `kicad-cli pcb render --quality high` (KiCad ≥ 9.0, 10.0.5
  known-good — CPU raytracer, no X server needed).
- `tools/turntable.sh` — 24-frame 360° orbit (`turntable.gif` per board) from
  the same raytracer, stitched with ffmpeg.
